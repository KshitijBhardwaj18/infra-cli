...
export async function runDeploy() {
    ...
    try {
        const { stdout } = await execa("aws", ["sts", "get-caller-identity", "--profile", cfg.awsProfile, "--output", "json"]);
    ...
    try {
        await execa("aws", ["s3api", "head-bucket", "--bucket", stateBucket, "--profile", cfg.awsProfile], { stderr: "ignore" });
    ...
    try {
        const { stdout } = await execa("pulumi", ["stack", "ls"], { env, cwd: infraDir });
        const hasStack = stdout.split("\n").some((l) => l.trim().startsWith(cfg.env));
        if (!hasStack) {
            await execa("pulumi", ["stack", "init", cfg.env], { env, cwd: infraDir });
        }
        else {
    ...
    await execa("pulumi", ["config", "set", "aws:region", cfg.region], { env, cwd: infraDir });
    console.log();
    ...
    let outputs = {};
    try {
        const { stdout } = await execa("pulumi", ["stack", "output", "--json"], { env, cwd: infraDir });
