...
import { CPU_PRESETS, DB_PRESETS, CACHE_PRESETS, NAT_PRESETS, NETWORKING_DEFAULTS, DATABASE_DEFAULTS, CACHE_DEFAULTS, ECS_DEFAULTS, REGIONS, ALB_MONTHLY_COST, STORAGE_OVERHEAD_MONTHLY, } from "../schema/presets.js";
...
export async function runInit(opts = {}) {
    ...
    if (opts.envOnly) {
        ...
        const envCfg = { awsProfile, secrets, env };
...
async function promptNetworking(region) {
    section("Networking");
    ...
    for (let i = 0; i < total; i++) {
        ...
        const name = await input({
            ...
            validate: (v) => {
                ...
                if (services.some((s) => s.name === v))
                    return "Service names must be unique.";
                return true;
            },
        });
        ...
        const cpuChoices = Object.keys(CPU_PRESETS).map((k) => ({
        ...
        if (type === "backend") {
            ...
            const cpu = (await select({ message: "CPU:", choices: cpuChoices }));
...
async function promptDataStores() {
    section("Data Stores");
    ...
    if (dbEngine === "postgres") {
        dbSize = (await select({
            message: "Size:",
            choices: Object.keys(DB_PRESETS).map((k) => ({
    ...
    if (cacheEngine === "redis") {
        cacheSize = (await select({
            message: "Size:",
            choices: Object.keys(CACHE_PRESETS).map((k) => ({
    ...
    while (true) {
        ...
        const name = await input({
            ...
            validate: (v) => {
                if (!isKebabCase(v))
                    return "Must be kebab-case.";
                if (manual.some((s) => s.name === v))
                    return "Already added.";
                if (generated.some((s) => s.name === v))
                    return "Already used by a generated secret.";
                return true;
            },
        });
        const envVar = await input({
...
async function promptEnvVars(cfg) {
    ...
    if (cfg.database.engine === "postgres")
        autoLines.push(`DATABASE_URL    = from RDS output     (auto)`);
    if (cfg.cache.engine === "redis")
        autoLines.push(`REDIS_URL       = from Redis output   (auto)`);
    ...
    for (const svc of cfg.services) {
        ...
        if (Object.keys(perService).length > 0)
            env[svc.name] = perService;
    }
    return env;
}
function printSummary(cfg, envCfg) {
    ...
    if (cfg.storage.enabled)
        console.log("    S3 bucket (versioning, AES-256, public access blocked)");
    console.log();
    header("  Secrets:");
