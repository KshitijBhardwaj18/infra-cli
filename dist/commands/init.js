...
async function promptDataStores(projectName) {
    section("Data Stores");
...
async function promptSecrets(services) {
    ...
    while (true) {
        ...
        if (kind === "generated") {
            ...
            const spec = { name, envVar, services: selected };
            if (value)
                spec.value = value;
            secrets.push(spec);
        }
...
function printSummary(cfg, envCfg) {
    ...
    if (cfg.storage.enabled)
        console.log("    S3 bucket (versioning, AES-256, private)");
    console.log();
    header("  Secrets:");
